 ...
 * VARIANTE DE PRODUÇÃO — uso exclusivo no cutover (ver docs/RUNBOOK_CUTOVER_PRODUCAO.md).
 *
 * Uso:
 *   pnpm tsx scripts/migrate-to-multi-tenant-producao.ts          (DRY-RUN, padrão)
 *   pnpm tsx scripts/migrate-to-multi-tenant-producao.ts --apply  (aplicação real)
 *
 * Pré-requisitos:
 *   - Sub-fases 3.7.1a, 3.7.1b, 3.7.1c já aplicadas no banco de produção
 *   - DB_NAME=d5ea2e96_solutegdb e DATABASE_URL no .env apontando para produção
 *   - Backup recente antes de usar --apply
 *
 * Referência de uso: docs/RUNBOOK_CUTOVER_PRODUCAO.md (Fase 7).
 */

import { config } from 'dotenv';
config();

import { assertProductionEnvironment, maskEmail } from '../server/lib/environment';
...
async function main() {
  ...
  // Aborta se não for produção (lê DB_NAME do .env)
  ...
  if (isApply) {
    ...
    console.log('    pnpm tsx scripts/migrate-to-multi-tenant-producao.ts --apply');
  }
