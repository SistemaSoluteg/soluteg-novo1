...
🟡 Fase 3.7 — Multi-tenant (EM ANDAMENTO — 3.7.2 + 3.7.1f CONCLUÍDAS E VALIDADAS EM STAGING: 9 routers isolados + travamento NOT NULL/FK + rotação JWT + **hardening pré-cutover concluído** (SEC-02/03, LAUDO-01, metrics — staging). Próximo: **cutover de produção** — produção ainda com zero do multi-tenant)
...
| 3.7.2 | Isolamento de queries (helper `forTenant`) — **mais crítica** | ✅ Concluída e **validada em staging** (9 routers incl. `laudos` com ghost-probe+regressão; SEC-01/SSE/`pdvProcedure`/`pushSubscriptions` cobertos; `reports`/`users` removidos; backfill script estende para todas as tabelas). Único ponto por partes: propagação SSE em tempo real precisa de teste no navegador com sensor real (MQTT off no staging) |
| 3.7.1f | `tenantId` NOT NULL + FKs + índices + rotacionar JWT_SECRET — **travamento final** | ✅ Concluída **em staging** (16/08): 41 tabelas travadas + FK, JWT rotacionado e validado (`notificationLogs` fora, NOTIF-01). Falta aplicar no **cutover de produção** |
...
- **16/08/2026** — **3.7.1f (travamento final) concluída em staging.** Fases 0-3 executadas e validadas no `_tst`: diagnóstico via `information_schema` (42 tabelas com `tenantId`); backfill sem NULL residual (0 deriváveis, staging já limpo); **39 `ALTER ... NOT NULL` + `ADD FOREIGN KEY → tenants(id)`** aplicados e validados um a um por `information_schema` (total 41 tabelas travadas com as 2 centrais da 3.7.1b; só `notificationLogs` fora — NOTIF-01); e **rotação do `JWT_SECRET`** validada dos dois lados (sessão antiga caiu na hora, login novo entrou). Scripts commitados (`diagnostico-3.7.1f-fase0.ts`, `lock-tenant-not-null-fk.ts`, `backfill-tenant-null.ts` estendido). Achado registrado: **LOCK-01** — tabelas legadas/infra (`admins`/`auditLog`/`invites`/`inspectionReports`) travadas com NOT NULL mas com writers mortos/inativos que não carimbam `tenantId`; ao reativar (3.7.4/3.7.7) o writer TEM que carimbar. **Falta:** o cutover de produção (produção ainda com zero do multi-tenant).
- **16/08/2026** — **3.7.2 plenamente validada em staging.** Rodados contra o banco `_tst` (servidor local em porta separada, JWT real, massa limpa no final): `laudos` — ghost-probe (laudo/foto do tenant 2 → NOT_FOUND), FK forjada (`clienteId`/`tecnicoId` de outro tenant → NOT_FOUND), regressão completa (criar → foto/medição/citação → atribuir técnico → PDF) e portal do técnico (só vê os seus); `adminMetrics` bateu com o banco (a rota antiga devolve o SPA, não 404 — a rota Express foi removida mesmo, é o catch-all do Vite); `pdvProcedure` tenant 1 OK; SSE `/api/water-tank-sse` 401 sem cookie / 200 com. Script de backfill estendido (16 regras via FK-pai, dry-run+apply zerou tudo derivável, `workOrderHistory` 5→0). **Achados registrados:** (1) `deleteLaudo` não limpa `laudoCitacoes` (bug de cascade, não bloqueia); (2) budget órfão `id=22` (lixo de teste, clientId inexistente) — deletar antes da 3.7.1f; (3) `notificationLogs` acumula NULL porque `notifications.ts` insere sem carimbar tenant — **excluir da NOT NULL da 3.7.1f**, corrigir o write-path na 3.7.9. Único teste por partes: propagação SSE ao vivo (MQTT off no staging) precisa de verificação no navegador com sensor real.
- **15/08/2026** — **3.7.2 (isolamento) CONCLUÍDA.** 9º router `laudos` isolado (commit `3dd4d4e`, Método B, 40+ endpoints) — guarda de posse por laudo, FK-do-input (`clienteId`/`tecnicoId`), sub-recursos via laudo pai carimbando tenant, `listLaudos` fail-closed. Na mesma leva: **SEC-01 fechado** (`GET /api/admin-metrics` sem auth → `adminMetrics.router` tRPC escopado por adminId+tenantId, commit `44bd66b`); gate **`pdvProcedure`** (PDV só tenant 1, commit `833834f`); SSE **`/api/water-tank-sse`** com `requireClientAuth` + clientId do JWT (gap que a auditoria do waterTankAdmin tinha perdido); **`pushSubscriptions`** carimba `tenantId` (commit `f5759ec`); **faxina dos legados `reports`/`users`** removidos sem quebrar `ctx.user`/`auth` (commit `3d0d653`); e o **script `scripts/backfill-tenant-null.ts`** iniciado (commit `5b3e962`).
...
- **20/08/2026** — **LOCK-02 (achado + correção).** Ao religar o MQTT no staging com o broker real de produção (sensor `sensor_01` ao vivo), descobriu-se que a `waterTankSensors` travada com `tenantId NOT NULL` na 3.7.1f quebrava **100% da ingestão MQTT** (sensores já atribuídos incluídos): `upsertSensorDevice()` faz `INSERT ... ON DUPLICATE KEY UPDATE` sem `tenantId` (auto-descoberta de sensor, global por design), e o MySQL exige o `NOT NULL` na cláusula `VALUES` mesmo caindo no ramo `ON DUPLICATE KEY UPDATE`. Revertida pra nullable e validada com leitura real voltando a salvar (`tenantId=1`, 1 leitura a cada ~30s). A `waterTankSensors` passa a ficar **fora** do NOT NULL no cutover de produção (mesma categoria do `notificationLogs`). Registrado como `LOCK-02` em `docs/PENDENCIAS_TECNICAS.md`.
- **21/08/2026** — **Hardening pré-cutover concluído e validado em staging.** Fechados: `SEC-02`/`SEC-03` (`POST /api/water-tank-monitoring` passou a resolver `adminId`/`tenantId` do cookie `admin_token` + guarda de posse do `clientId`; commit `baf10a5`), `LAUDO-01` (`deleteLaudo` limpa `laudoCitacoes`; commit `67d1349`) e o isolamento de `workOrders.metrics` por tenant (10 funções via `sql\`\`` parametrizado + `ctx.tenantId`; commit `abdc40a`). Validação: `tsc` 32/zero novos no build de staging; dashboard "Métricas de OS" = 83 OS do tenant 1; laudo com 2 citações deletado → 0 órfãs. Achado: `POST /api/water-tank-monitoring` **não tem caller no frontend** (endpoint só de API — candidato a faxina; o hardening ali é defesa em profundidade). Próximo marco: **cutover de produção**.
