...
🟡 Fase 3.7 — Multi-tenant (EM ANDAMENTO — 3.7.2: `technicians`, `clients`, `workOrders` e `budgets` isolados e validados em staging)
...
| 3.7.2 | Isolamento de queries (helper `forTenant`) — **mais crítica** | 🟡 EM ANDAMENTO (`technicians`, `clients`, `workOrders` e `budgets` isolados; próximo: a definir) |
