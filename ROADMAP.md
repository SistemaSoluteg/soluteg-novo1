...
🟡 Fase 3.7 — Multi-tenant (EM ANDAMENTO — 3.7.2: 8 routers isolados e validados: `technicians`, `clients`, `workOrders`, `budgets`, `checklists`, `technicianPortal`, `documents`, `waterTankAdmin` + faxina de código morto)
...
| 3.7.2 | Isolamento de queries (helper `forTenant`) — **mais crítica** | 🟡 EM ANDAMENTO (8 routers isolados incl. `waterTankAdmin`; varredura completa: sobra só o gap de `clientProfile.uploadPhoto` + verificar legados `reports`/`users`/`pushSubscriptions`) |
