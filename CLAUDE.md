...
> ## ✅ 22/08/2026 — PRODUÇÃO JÁ RODA MULTI-TENANT (cutover Fases 1–9 concluídas)
> **Fases 1–9 do [`docs/RUNBOOK_CUTOVER_PRODUCAO.md`](./docs/RUNBOOK_CUTOVER_PRODUCAO.md) CONCLUÍDAS (22/08).** Código multi-tenant **deployado em produção** (`c9d926e`), schema aplicado, **dados migrados** (tenants `jnc` id=1 / `soluteg-direto` id=2, platformAdmin Thiago id=1), backfill fechado — **banco 100% consistente: 0 `tenantId` NULL nas 42 tabelas**. Login admin/cliente/técnico validado no ar; caminhos de escrita fail-closed (job mensal, alertas) funcionando; código novo carimba `tenantId` na ingestão MQTT. **`deploy-app` voltou a ser SEGURO** (schema + dados + código alinhados) — o reflexo "commit→push→deploy" está normalizado de novo.
> **Faltam só passos de endurecimento (não urgentes, sistema estável sem eles):** Fase 10 (NOT NULL + FK — `scripts/lock-tenant-not-null-fk-producao.ts`, exclui `notificationLogs`/`waterTankSensors`/`client_equipment`; inclui `admins`/`auditLog`/`invites`/`inspectionReports` por LOCK-01), Fase 11 (rotação do `JWT_SECRET` — desloga todos), Fase 12 (smoke tests), Fase 13 (docs finais).
> **Achados do cutover (todos resolvidos, no runbook):** Fase 6 — o `#` de `'#D4A84B'` quebrou a `CREATE TABLE tenants` no splitter do DBeaver (rodar isolado); DBeaver modo Record escondia linhas; 6a ficou pra trás (§8). Fase 7 — bug cosmético "0 linhas" no `--apply` (§9). **Fase 8 — `admins.tenantId` não existia em produção** (foi manual em staging na 3.7.2, nunca virou migration; o `context.ts` lê `admins.tenantId` no login admin) → corrigido com `ALTER`+`UPDATE` antes do deploy (§10.8.0). Migração/backfill rodaram do checkout de staging com `cwd` no dir de produção (árvore do dir de produção estava suja) — depois do `git pull` do deploy os scripts já ficam no próprio dir de produção.
...
> ✅ **Seguro de novo** (22/08/2026) — o cutover multi-tenant (Fases 1–9) foi concluído; schema, dados e código estão alinhados em produção. `deploy-app`/este bloco voltaram a ser rotina normal. Ver seção 2.
