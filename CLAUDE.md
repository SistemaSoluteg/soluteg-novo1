...
> **Cutover em andamento — Fases 1–6 do [`docs/RUNBOOK_CUTOVER_PRODUCAO.md`](./docs/RUNBOOK_CUTOVER_PRODUCAO.md) CONCLUÍDAS (22/08).** O merge foi oficializado (`master` e `multi-tenant` em `54757d3` — **`master` já contém todo o código do isolamento multi-tenant**), e o **schema multi-tenant já está aplicado no banco de produção** (6a auditoria + 6b tabelas centrais + 6c `tenantId` nullable em 38 tabelas; validado: 3 tabelas de auditoria + 41 colunas `tenantId`). **MAS os dados ainda não foram migrados** (Fase 7: criar tenants + carimbar `tenantId=1`) e o código antigo ainda roda em produção. **Rodar `deploy-app` agora ainda derruba a produção** — o código novo resolve `ctx.tenantId` a cada request e não há tenant/linha carimbada ainda. **NÃO rodar `deploy-app`** até completar as Fases 7–9 do runbook (migração de dados → deploy → backfill). Isso vale até o runbook ser marcado como concluído nesta seção. Percalços da Fase 6 (o `#` do `primaryColor` quebrando a `CREATE TABLE tenants` no splitter do DBeaver; DBeaver em modo Record escondendo linhas; a 6a que ficou pra trás) estão registrados no runbook §8.
