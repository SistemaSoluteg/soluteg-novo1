...
> ## 🚨 22/08/2026 — CUTOVER EM ANDAMENTO — `deploy-app` só na Fase 8 coordenada
> **Fases 1–7 do [`docs/RUNBOOK_CUTOVER_PRODUCAO.md`](./docs/RUNBOOK_CUTOVER_PRODUCAO.md) CONCLUÍDAS (22/08).** Merge oficializado (`master`/`multi-tenant` em `54757d3` — **`master` já tem todo o código do isolamento**), **schema multi-tenant aplicado no banco de produção** (6a auditoria + 6b centrais + 6c `tenantId` nullable: 3 tabelas de auditoria + 41 colunas `tenantId`), e **dados migrados** (tenants `jnc` id=1 / `soluteg-direto` id=2, platformAdmin Thiago id=1, `tenantId=1` em ~160k linhas — validado, zero NULL residual nas tabelas estáveis). **Agora o banco JÁ está pronto pro código novo.** O próximo passo é a **Fase 8 (deploy)** — que é justamente rodar o deploy do código; mas tem que ser **coordenado com a Fase 9 (backfill dos órfãos que o código antigo ainda está criando)**, não o reflexo automático "commit→push→deploy". Enquanto o código antigo roda, tabelas de escrita ao vivo (`waterTankMonitoring`, `notificationLogs`) acumulam `tenantId=NULL` — normal, a Fase 9 limpa depois do deploy. **NÃO rodar `deploy-app` solto**: seguir o runbook Fases 8→9→10→11. **🚨 Pré-requisito bloqueante da Fase 8 (achado 22/08):** `admins.tenantId` **não existe em produção** — foi criada manualmente em staging na 3.7.2, nunca virou migration, e o `context.ts` lê `admins.tenantId` no login admin. Deployar sem ela = admin loga mas cai em fail-closed em toda operação. Corrigir antes do deploy: `ALTER TABLE admins ADD COLUMN tenantId int NULL AFTER id;` + `UPDATE admins SET tenantId=1 WHERE tenantId IS NULL;` (detalhe no runbook §10.8.0). `clients`/`technicians` (as outras fontes do context) já foram carimbadas na Fase 7. Percalços já registrados: Fase 6 (o `#` do `primaryColor` quebrando a `CREATE TABLE tenants` no splitter do DBeaver; DBeaver modo Record; 6a que ficou pra trás) no runbook §8; Fase 7 (bug cosmético "0 linhas" no `--apply`) no §9. **Como o dir de produção estava com árvore suja, a migração rodou o script do checkout de staging com `cwd` no dir de produção** — a Fase 8 vai precisar resolver isso pro `git pull` do deploy (ver runbook §10/Fase 8).
...
- **Excluir `waterTankSensors` do NOT NULL (LOCK-02, achado 20/08)** — `upsertSensorDevice()` (`server/waterTankSensorDb.ts:12`) faz `INSERT ... ON DUPLICATE KEY UPDATE` sem `tenantId` (auto-descoberta de sensor MQTT, GLOBAL POR DESIGN — sensor novo não tem tenant até um admin atribuir). O MySQL exige que o `INSERT` satisfaça o `NOT NULL` mesmo quando cai no ramo `ON DUPLICATE KEY UPDATE` — travar essa tabela quebrou **100% da ingestão MQTT em staging**, sensores já atribuídos incluídos, descoberto ao religar o MQTT no staging com o broker real de produção (sensor `sensor_01` ao vivo). Revertida manualmente (`DROP FOREIGN KEY` + `MODIFY COLUMN tenantId INT NULL`) e validada com leitura real voltando a salvar. Corrigir o código de verdade (carimbar tenant quando o sensor já está atribuído) fica como dívida técnica — não bloqueia o cutover, só não trava essa tabela nele.
